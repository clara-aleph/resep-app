...
export function AddRecipeCard({ categories, onCreated }: { categories: string[]; onCreated: (recipe: Recipe) => void }) {
  ...
  return <section className="w-full max-w-2xl justify-self-start rounded-3xl border border-orange-100 bg-white p-5 shadow-sm sm:p-7 md:max-w-none md:rounded-none md:border-x-0 md:border-t-0 md:border-b-2 md:bg-transparent md:px-0 md:pb-10 md:pt-0 md:shadow-none">
    <div className="mb-6"><h2 className="text-3xl font-bold text-stone-900">{t("addRecipe")}</h2></div>
    <div className="mb-5 grid grid-cols-2 rounded-xl bg-orange-50 p-1 md:bg-stone-200">
      <button onClick={() => { setMode("tautan"); setSourcePhoto(null); }} className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === "tautan" ? "bg-white text-orange-700 shadow-sm md:bg-stone-800 md:text-white" : "text-stone-600 md:bg-stone-200 md:text-stone-700"}`}>{t("saveVideoLink")}</button>
      <button onClick={() => setMode("foto")} className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === "foto" ? "bg-white text-orange-700 shadow-sm md:bg-stone-800 md:text-white" : "text-stone-600 md:bg-stone-200 md:text-stone-700"}`}>{t("uploadRecipePhoto")}</button>
    </div>
    {mode === "tautan" ? <div className="w-full"><div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder={t("videoPlaceholder")} className="w-full rounded-xl border border-stone-300 bg-white px-3 py-2.5 text-sm outline-orange-400 md:flex-1"/><button onClick={() => ambilTautan()} disabled={loading} className="w-full rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60 md:w-auto md:shrink-0">{t("analyzeVideo")}</button></div>{loading && status && <p role="status" className="mt-3 text-sm text-stone-600"><LoaderCircle className="mr-1 inline animate-spin" size={15}/> {status}</p>}</div> : <div><label className={`flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-orange-300 bg-orange-50 px-4 py-6 text-sm font-semibold text-orange-800 md:bg-white ${isAnalyzingPhoto ? "pointer-events-none opacity-60" : ""}`}><ImageUp size={18}/>{t("chooseRecipePhoto")}<input type="file" accept="image/*" onChange={bacaFoto} disabled={isAnalyzingPhoto} className="hidden" /></label>{isAnalyzingPhoto && status && <p role="status" className="mt-3 text-sm text-stone-600"><LoaderCircle className="mr-1 inline animate-spin" size={15}/> {status}</p>}</div>}
    <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">{t("recipeTitle")}<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder={t("titlePlaceholder")} className="mt-1.5 w-full rounded-xl border border-stone-300 bg-white px-3 py-2.5 outline-orange-400"/></label><label className="text-sm font-medium">{t("category")}<div className="mt-1.5"><CategoryCombobox value={category} onChange={setCategory} categories={categories}/></div></label></div>
    <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">{t("ingredients")}<textarea value={ingredients} onChange={(event) => setIngredients(event.target.value)} placeholder={t("oneIngredient")} rows={4} className="mt-1.5 w-full resize-none rounded-xl border border-stone-300 bg-white px-3 py-2.5 outline-orange-400"/></label><label className="text-sm font-medium">{t("instructions")}<textarea value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder={t("oneStep")} rows={4} className="mt-1.5 w-full resize-none rounded-xl border border-stone-300 bg-white px-3 py-2.5 outline-orange-400"/></label></div>
    ...
    <button onClick={simpan} disabled={loading} className="mt-5 w-full rounded-xl bg-orange-600 px-4 py-3 font-semibold text-white hover:bg-orange-700 disabled:opacity-60">{t("saveRecipe")}</button>
  </section>;
}
