...
export function AddRecipeCard({ categories, onCreated }: { categories: string[]; onCreated: (recipe: Recipe) => void }) {
  ...
  return <section className="w-full max-w-2xl justify-self-start rounded-3xl border border-orange-100 bg-white p-5 shadow-sm sm:p-7">
    ...
    {mode === "tautan" ? <div className="w-full"><div className="flex gap-2"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="Tempel tautan video YouTube, Facebook, atau Instagram publik..." className="min-w-0 flex-1 rounded-xl border border-stone-300 px-3 py-2.5 text-sm outline-orange-400"/><button onClick={() => ambilTautan()} disabled={loading} className="rounded-xl bg-stone-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">Analisis Video</button></div>{loading && status && <p role="status" className="mt-3 text-sm text-stone-600"><LoaderCircle className="mr-1 inline animate-spin" size={15}/> {status}</p>}</div> : <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-orange-300 bg-orange-50 px-4 py-6 text-sm font-semibold text-orange-800"><ImageUp size={18}/>Pilih foto catatan resep<input type="file" accept="image/*" onChange={bacaFoto} className="hidden" /></label>}
