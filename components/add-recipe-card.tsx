...
export function AddRecipeCard({ categories, onCreated }: { categories: string[]; onCreated: (recipe: Recipe) => void }) {
  ...
  return <section className="w-full max-w-2xl justify-self-start rounded-3xl border border-orange-100 bg-white p-5 shadow-sm sm:p-7 md:max-w-none md:rounded-none md:border-x-0 md:border-t-0 md:border-b-2 md:bg-transparent md:px-0 md:pb-10 md:pt-0 md:shadow-none">
    ...
    <div className="mb-5 grid grid-cols-2 rounded-xl bg-orange-50 p-1 md:bg-stone-200"><button onClick={() => { setMode("tautan"); setSourcePhoto(null); }} className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === "tautan" ? "bg-white text-orange-700 shadow-sm md:bg-stone-800 md:text-white" : "text-stone-600 md:bg-stone-200 md:text-stone-700"}`}>Simpan Tautan Video</button><button onClick={() => setMode("foto")} className={`rounded-lg px-3 py-2 text-sm font-semibold ${mode === "foto" ? "bg-white text-orange-700 shadow-sm md:bg-stone-800 md:text-white" : "text-stone-600 md:bg-stone-200 md:text-stone-700"}`}>Unggah Foto Resep</button></div>
    {mode === "tautan" ? <div className="w-full"><div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="Tempelkan tautan video YouTue...." className="w-full rounded-xl border border-stone-300 bg-white px-3 py-2.5 text-sm outline-orange-400 md:flex-1"/><button onClick={() => ambilTautan()} disabled={loading} className="w-full rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60 md:w-auto md:shrink-0">Analisis Video</button></div>{loading && status && <p role="status" className="mt-3 text-sm text-stone-600"><LoaderCircle className="mr-1 inline animate-spin" size={15}/> {status}</p>}</div> : <div><label className={`flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-orange-300 bg-orange-50 px-4 py-6 text-sm font-semibold text-orange-800 md:bg-white ${isAnalyzingPhoto ? "pointer-events-none opacity-60" : ""}`}><ImageUp size={18}/>Pilih foto catatan resep<input type="file" accept="image/*" onChange={bacaFoto} disabled={isAnalyzingPhoto} className="hidden" /></label>{isAnalyzingPhoto && status && <p role="status" className="mt-3 text-sm text-stone-600"><LoaderCircle className="mr-1 inline animate-spin" size={15}/> {status}</p>}</div>}
    <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">Judul resep<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Contoh: Sayur Labu Santan" className="mt-1.5 w-full rounded-xl border border-stone-300 bg-white px-3 py-2.5 outline-orange-400"/></label><label className="text-sm font-medium">Kategori<div className="mt-1.5"><CategoryCombobox value={category} onChange={setCategory} categories={categories}/></div></label></div>
    <div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">Bahan-bahan<textarea value={ingredients} onChange={(event) => setIngredients(event.target.value)} placeholder="Satu bahan per baris" rows={4} className="mt-1.5 w-full resize-none rounded-xl border border-stone-300 bg-white px-3 py-2.5 outline-orange-400"/></label><label className="text-sm font-medium">Langkah-langkah<textarea value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder="Satu langkah per baris" rows={4} className="mt-1.5 w-full resize-none rounded-xl border border-stone-300 bg-white px-3 py-2.5 outline-orange-400"/></label></div>
