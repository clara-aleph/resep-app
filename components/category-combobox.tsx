...
export function CategoryCombobox({ value, onChange, categories = [] }: { value: string; onChange: (value: string) => void; categories?: string[] }) {
  ...
  return <div className="relative">
    <button
      ...
      aria-expanded={open}
    >
      <span className={value ? "" : "text-stone-400"}>{value ? tampilkanKategori(value) : t("categoryPlaceholder")}</span>
      <ChevronDown size={16} />
    </button>
    {open && <Command className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border border-stone-200 bg-white p-1 shadow-lg">
      <Command.Input value={query} onValueChange={setQuery} placeholder={t("categoryPlaceholder")} className="w-full border-b border-stone-100 px-3 py-2 text-sm outline-none" />
      <Command.List onTouchStart={mulaiSentuhan} onTouchMove={gerakkanSentuhan} onKeyDown={() => { touchDidMoveRef.current = false; }} className="max-h-48 overflow-auto py-1">
        <Command.Empty className="px-3 py-2 text-sm text-stone-500">{t("noMatchingCategory")}</Command.Empty>
        {normalized && !hasExact && <Command.Item value={`buat-${normalized}`} onSelect={() => pilihDariDaftar(normalized)} onClick={() => pilihDariDaftar(normalized)} className="cursor-pointer rounded-lg px-3 py-2 text-sm aria-selected:bg-orange-50">{t("createCategory")} “{normalized}”</Command.Item>}
