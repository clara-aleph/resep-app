...
function parseRecipe(rawResponse: string) {
  ...
  try {
    ...
    const title = typeof entries.find(([key]) => key === "title" || key.includes("judul"))?.[1] === "string" ? String(entries.find(([key]) => key === "title" || key.includes("judul"))?.[1]).trim() : "";
    ...
    if (title || bahan.length || cara_membuat.length) return { title, bahan, cara_membuat };
